(function () {
  ...
  const promoCodesEncoded = {
    "TklDVURTMzc=": { used: true, reward: () => addBalance(100) },
    "R0lGVEVNTQ==": { used: false, reward: () => addCase("gift") },
    "U1BJTkNPSU4=": { used: true, reward: () => alert("Спіни ще не реалізовано") },
    "T1BFTkJBVExQQVM=": { used: true, reward: () => alert("Батл пас буде додано") },
    "TFZMMQ==": { used: true, reward: () => alert("Рівень батл пасу буде додано") },
    "RE9OQVRJNDQ4": { used: false, unlimited: true, reward: () => addBalance(100) },
    "SEFMQVZB MTAwMA==": { used: false, reward: () => addBalance(1000) },
    "SEFMQVZBWFhY": { used: false, unlimited: true, reward: () => addBalance(1000) },
    "UkVBTEdJRlQ=": { used: false, unlimited: true, reward: () => addCase("gift") }
  };

  const promoCodes = {};
  for (const encodedKey in promoCodesEncoded) {
    const key = b64DecodeUnicode(encodedKey);
    promoCodes[key] = promoCodesEncoded[encodedKey];
  }
  ...
  window.toggleBlockItem = toggleBlockItem;
  window.addToCart = addToCart;
  window.showCart = showCart;
  window.removeFromCart = removeFromCart;
  window.buyCartItems = buyCartItems;
  window.openCase = openCase;
  window.goToPromoMenu = goToPromoMenu;
  window.usePromoFromInput = usePromoFromInput;
